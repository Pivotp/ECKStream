...
fl = pd.read_csv('Surveillance.csv', low_memory=False)
V = fl.columns[0:]


def Event_triggered(epsilon):
    file_path = 'Surveillance.csv'
    ...
    for node in V:
        ...
        current_time = pre_trigger_index[node]+1
...
def noise_data(U, df):
    ...
    for node in V:
        ...
        error = np.abs(orig_aligned - perturbed_aligned)
        mae_error[node] = np.sum(error) / len(orig_aligned)

        euclidean_distance[node] = np.sqrt(np.sum((orig_aligned - perturbed_aligned) ** 2))
...
def main():
    ...
    with open("Surveillance/SET_pretuber_result", "a") as f:
        ...
        for ep in np.arange(min_epsilon, max_epsilon, step_epsilon):
            ...
            for run in range(num_runs):
                df = pd.read_csv('Surveillance.csv', low_memory=False)
                perturb_data = Event_triggered(ep)
